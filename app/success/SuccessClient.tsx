...
export default function SuccessClient() {
  ...
  useEffect(() => {
    ...
    setOrderId(fromUrl ? fromUrl : `SR-${Date.now()}`);
  }, [params]);
  ...
  const availability = useMemo(() => {
    if (!mounted) return { used: 0, capacity: branch.boxCapacity };
    const used = getUsed(branchId, slot);
    return { used, capacity: branch.boxCapacity };
  }, [mounted, branchId, slot, branch.boxCapacity]);
      ...
      <div className={styles.grid}>
        <div className={styles.panel}>
          ...
          <div className={styles.meta}>
            ...
            <div className={styles.metaLine}>
              <span className={styles.metaKey}>Box limiti:</span> <b>{availability.used}/{availability.capacity}</b>
            </div>
          </div>
        ...
        <div className={styles.panel}>
          ...
          <div className={styles.note}>
            {handover === "box" ? "QR box-u açır (demo)." : "Box yoxdur, satıcı QR ilə təhvil verir (demo)."}
          </div>
        </div>
      </div>
    </div>
